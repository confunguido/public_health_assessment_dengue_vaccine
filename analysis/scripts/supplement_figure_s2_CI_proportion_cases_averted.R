...
#=============================================================================#
summary_list_test = readRDS('../data/output/20190218_output/summary_routine_vaccination_All_sweep_files_test.RDS')
...
if(save.fig == T){
  jpeg(sprintf('../figures/supplement_figure_S2_CI_cases_averted_heatmap_10y.jpeg'),
...
for(sp9_tmp in sp9_array){
  ...
  cases_averted_prediction_test_novax = data.frame(
    ...
    'Age' = age, 'SP9' = sp9_tmp, 'Coverage' = coverage)
  if(m=="randomforest"){
    cases.prediction.grid = calculate_ci_randomForest(model_cases_novax,cases_averted_prediction_test_novax)
    hosp.prediction.grid = calculate_ci_randomForest(model_hosp_novax,cases_averted_prediction_test_novax)    
  }else if(m == "gam"){
    cases.prediction.grid = calculate_ci_gam(model_cases_novax,cases_averted_prediction_test_novax)
    hosp.prediction.grid = calculate_ci_gam(model_hosp_novax,cases_averted_prediction_test_novax)    
  }
  ...
  for(sen in 1:length(sensitivity_array)){
    for(spec in 1:length(specificity_array)){
      cases_averted_matrix_test_novax[sen,spec] = cases.prediction.grid$CI[k]
      hosp_averted_matrix_test_novax[sen,spec] = hosp.prediction.grid$CI[k]
      k = k + 1
    }
  }
  # correct bounds
...
plot_color_bar(myColStrip_in = myColStrip, 
               ...
               myTicks_in = myColBreaksTicks, label_in = "CI of proportion of cases averted", 
               cex = 0.6,line = 1.5,plot_line=FALSE)
