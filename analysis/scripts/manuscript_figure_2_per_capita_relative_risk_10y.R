...
for(ff in 1:length(summary_list)){
  ...
  cohort_dataframe_test$DisAverted[ff] = sum(cohort_table$DisVax, na.rm = T) / sum(cohort_table$DisNoVax, na.rm = T)
  cohort_dataframe_test$HospAverted[ff] = sum(cohort_table$HospVax, na.rm = T) / sum(cohort_table$HospNoVax, na.rm = T)
...
cohort_dataframe_test = cohort_dataframe_test[!is.infinite(cohort_dataframe_test$HospAverted),]
...
if(m == "randomforest"){
  n.tree = 1000
  model_dis_test = randomForest(
    DisAverted ~
      (Sensitivity + Specificity  + SP9 ) ^ 3,
    data = as.data.frame(cohort_dataframe_test),
    importance = T,
    ntree = n.tree)
  
  model_hosp_test = randomForest(
    HospAverted ~
      (Sensitivity + Specificity  + SP9 ) ^ 3,
    data = as.data.frame(cohort_dataframe_test),
    importance = T,
    ...
    cohort_dataframe_test$DisAverted = cohort_dataframe_test$DisAverted + 1e-5
    cohort_dataframe_test$HospAverted = cohort_dataframe_test$HospAverted + 1e-5
    model_dis_test = gam(
        log(DisAverted) ~
...
if(save.fig == T){
    if(save.tiff == T){
        tiff(sprintf('../figures/manuscript_figure_2_per_capita_risk_10y.tiff'),
             width=6.35,height=2.5,units='in',res=300)
    }else{
        jpeg(sprintf('../figures/manuscript_figure_2_per_capita_risk_10y.jpeg'),
...
for(sp9_tmp in sp9_array){
  ...
  if(m == "randomforest"){
    cases.prediction.grid =  predict(
      model_dis_test, cases_averted_prediction_test, predict.all = F)
    hosp.prediction.grid =  predict(
      model_hosp_test, cases_averted_prediction_test, predict.all = F)
  }else if (m == "gam"){
    cases.prediction.grid =  exp(predict(
      model_dis_test, cases_averted_prediction_test))
    hosp.prediction.grid =  exp(predict(
      model_hosp_test, cases_averted_prediction_test))
  }
