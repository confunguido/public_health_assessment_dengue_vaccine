...
if(m == "randomforest"){
  ...
  model_propvax_test = randomForest(PropVax ~ (
    Specificity + Sensitivity + SP9)^3,
    data = cohort_dataframe_test,ntree = 1000, importance = T)
}else{
...
cost_effective_matrix_test_screening = matrix(0,nrow = length(sensitivity_array), 
...
if(save.fig == T){
    if(save.tiff == T){
        tiff(sprintf('../figures/manuscript_figure_5_cohort_%.0f_age_%.0f_coverage_%.0f_%s_10y.tiff',cc,9,80, country),
                             width=6.35,height=2.5,units='in',res=300)
    }else{
        jpeg(sprintf('../figures/manuscript_figure_5_cohort_%.0f_age_%.0f_coverage_%.0f_%s_10y.jpeg',cc,9,80, country),
...
for(dd in 1:length(daly.thresholds)){
  ...
  for(SP9_tmp in c(0.1,0.3,0.5,0.7,0.9)){
    ...
    prediction.grid =  predict(model_cost_treat_test,test_data,predict.all = F) + 
      predict(model_daly_test, test_data,predict.all = F) * daly.threshold.tmp - econ$c.vax * prop.vax
    k = 1
