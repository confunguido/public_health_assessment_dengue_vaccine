...
if(save.fig == T){
    if(save.tiff == T){
        tiff(sprintf('../figures/manuscript_figure_6_Individual_cost_test_%d_vax_%d_%s.tiff',econ$c.test,econ$c.vax,country),
             width=6,height=2.5,units='in',res=300)
    }else{
        jpeg(sprintf('../figures/manuscript_figure_6_Individual_cost_test_%d_vax_%d_%s.jpeg',econ$c.test,econ$c.vax,country),
