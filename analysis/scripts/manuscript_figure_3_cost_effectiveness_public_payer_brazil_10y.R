...
myColBar[myColBreaks[-(n.breaks+1)] < 0] = "#E0E0E0FF"
myColBreaksTicks = c(lb, seq(0,ub,by = (ub - lb)/10))
myColStrip = as.matrix((myColBreaks + diff(myColBreaks)[1] / 2)[-length(myColBreaks)])

if(save.fig == T){
    if(save.tiff == T){
        tiff(sprintf('../figures/manuscript_figure_3_%s_cost_per_thresholds_%s_10y.tiff', perspective,country),
             width=6.35,height=2.5,units='in',res=300)
    }else{
        jpeg(sprintf('../figures/manuscript_figure_3_%s_cost_per_thresholds_%s_10y.jpeg', perspective,country),
             width=6.35,height=2.5,units='in',res=400)
    }
}

layout(
  cbind(matrix(1:10,2,5,byrow = T),c(11,11)), widths = c(rep(2,5),1), heights = rep(1,6)
)
...
for(dd in 1:length(daly.thresholds)){
  ...
  for(SP9_tmp in c(0.1,0.3,0.5,0.7,0.9)){
    ...
    prop.vax = predict(model_propvax_test,test_data,predict.all = F)
    treatment.costs.p =  predict(model_cost_treat_test,test_data,predict.all = F)/N.tested
    dalys.lost.h.p = (
        predict(model_daly_test, test_data,predict.all = F)
    ) / N.tested

    dalys.lost.d.p= (
        predict(model_death, test_data,predict.all = F)
    ) / N.tested
        
    prediction.grid = treatment.costs.p +
        daly.threshold.tmp * (dalys.lost.h.p + dalys.lost.d.p) -
        prop.vax * econ$c.vax
    ...
    breaks.array = c(seq(lb,0, by = 5), seq(5,ub,by = 5))
    contour(
      x=specificity_array,
      y=sensitivity_array,
      z=t(cost_matrix),
      level=breaks.array,
      lwd= c(rep(0.1,length(which(breaks.array <0))), 1, rep(0.1,length(which(breaks.array > 0)))),
      c(rep(0.1,n.breaks/2),2,rep(0.1,n.breaks/2)),
      add=T,drawlabels=T
    )
    if(SP9_tmp == 0.9){
      mtext(text = sprintf("%.0fx GDP",gdp.vec[dd]), side = 4, line = 0, cex = 0.7)    
    }
    if(dd == 1){
...
plot_color_bar(myColStrip_in = myColStrip, 
                          ...
                          myTicks_in = myColBreaksTicks, myBarLims_in = c(lb,ub),
                          label_in = sprintf("Threshold cost of serological screening (USD)"), 
                          line = 1.5,cex = 0.6)
