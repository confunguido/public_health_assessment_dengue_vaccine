...
if(save.fig == T){
    if(save.tiff == T){
        tiff(sprintf('../figures/manuscript_figure_4_%s_cost_test_%d_vax_%d_%s_10y.tiff', 
                     perspective,econ$c.test,econ$c.vax,country),
             width=6,height=2.5,units='in',res=300)
    }else{
        jpeg(sprintf('../figures/manuscript_figure_4_%s_cost_test_%d_vax_%d_%s_10y.jpeg', 
                     perspective,econ$c.test,econ$c.vax,country),
